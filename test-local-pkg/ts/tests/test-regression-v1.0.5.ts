 ...
 *
 * Runs against the LOCAL SDK tarball (file:../../ts/imbrace-sdk-1.0.3.tgz),
 * so this catches regressions BEFORE publishing to npm. Use the published-
 * version test-api-pkg / test-accesstoken-pkg variants for post-publish
 * verification.
 *
 * Auth is auto-picked by utils/utils.ts:
 *   - IMBRACE_ACCESS_TOKEN set → Access Token auth
 *   - else IMBRACE_API_KEY    → API Key auth
 *
 * Each section pins one fix. If a regression sneaks back in, the
 * corresponding step fails with a concrete reason (not just "backend 400")
 * so it's easy to map back to the offending fix.
 *
 ...
 *   8. workflows.listInvitations — runtime smoke (Py-only Literal-hint fix).
 ...
 */
import { client, organizationId } from "../utils/utils.js"
...
const authMode = process.env.IMBRACE_ACCESS_TOKEN ? "ACCESS TOKEN" : "API KEY"
console.log(`\n━━━ v1.0.5 regression — auth: ${authMode} (local @imbrace/sdk tarball) ━━━`)
...
// SDK returned that verbatim though the type said BoardField.
...
// Py-only fix; TS already had the union type at compile time. Runtime smoke only.
