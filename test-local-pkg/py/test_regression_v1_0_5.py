...
Runs against the LOCAL imbrace SDK (editable install of ../../py), so this
catches regressions BEFORE publishing to PyPI. Use the published-version
test-api-pkg/py + test-accesstoken-pkg/py variants for post-publish verify.

Auth is auto-picked by utils.py:
...
from utils import client, ORG_ID, AUTH_MODE
...
print(f"\n=== v1.0.5 regression - auth: {AUTH_MODE} (local imbrace SDK, editable install) ===")
print(f"org={ORG_ID}")
