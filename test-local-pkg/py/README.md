# Imbrace SDK — Local Integration Test Suite (Python)

Bộ công cụ kiểm thử tích hợp (Integration Test) dành cho Imbrace Python SDK, xác thực mã nguồn trực tiếp qua file wheel cục bộ (`.whl`) trước khi phát hành lên PyPI.

---

## 1. Mục tiêu

- Xác thực toàn bộ logic SDK trước khi phát hành lên PyPI.
- Đảm bảo tính ổn định của các luồng nghiệp vụ quan trọng (Full Flow).
- Kiểm tra khả năng tương thích gói đóng gói cục bộ.
...
**Bước 1** — Build SDK local (từ thư mục gốc `py/`):
```bash
cd ../../py
python -m build
```

**Bước 2** — Cài đặt dependencies + SDK wheel:
```bash
...
├── requirements.txt      # Dependencies (imbrace wheel + pytest + dotenv)
├── utils/
