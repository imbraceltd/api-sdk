...
class CategoriesResource:
    """Categories domain — Sync. GET /3rd/categories"""
    ...
    def list(self, organization_id: str) -> List[Dict[str, Any]]:
        return self._http.request(
            "GET", self._base, params={"organization_id": organization_id}
        ).json()

    def get(self, category_id: str) -> Dict[str, Any]:
        return self._http.request("GET", f"{self._base}/{category_id}").json()

    def create(self, organization_id: str, name: str, apply_to: Optional[List[str]] = None, description: Optional[str] = None) -> Dict[str, Any]:
...
class AsyncCategoriesResource:
    ...
    async def create(self, organization_id: str, name: str, apply_to: Optional[List[str]] = None, description: Optional[str] = None) -> Dict[str, Any]:
        body: Dict[str, Any] = {"name": name}
        if apply_to is not None:
            body["apply_to"] = apply_to
        if description is not None:
    ...
    async def update(self, organization_id: str, category_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        res = await self._http.request(
            "PUT", f"{self._base}/{category_id}",
            json=body,
            headers={"organization_id": organization_id},
