...
class ContactsResource:
    ...
    def search(self, query: str) -> Dict[str, Any]:
        return self._http.request("GET", f"{self._base}/contacts/_search", params={"q": query}).json()
    ...
    def mark_notifications_read(self, notification_ids: list) -> Dict[str, Any]:
        return self._http.request("PUT", f"{self._base}/notifications/read",
                                  json={"notification_id": notification_ids}).json()
...
class AsyncContactsResource:
    ...
    async def get_conversations(self, contact_id: str, channel_types: Optional[str] = None) -> Dict[str, Any]:
        ...
        if channel_types:
            params["channel_types"] = channel_types
        res = await self._http.request("GET", f"{self._base}/contacts/{contact_id}/conversations",
                                       params=params)
        return res.json()

    async def list_notifications(self, limit: int = 20, skip: int = 0) -> Dict[str, Any]:
        res = await self._http.request("GET", f"{self._base}/notifications",
                                       params={"limit": limit, "skip": skip})
        return res.json()

    async def mark_notifications_read(self, notification_ids: list) -> Dict[str, Any]:
        res = await self._http.request("PUT", f"{self._base}/notifications/read",
                                       json={"notification_id": notification_ids})
        return res.json()
