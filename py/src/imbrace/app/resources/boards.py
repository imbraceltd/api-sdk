...
class BoardsResource:
    """Boards domain — App Gateway. /v1/backend/board"""
...
class AsyncBoardsResource:
    ...
    async def search(self, board_id: str, q: Optional[str] = None, limit: int = 100) -> Dict[str, Any]:
        body: Dict[str, Any] = {"limit": limit}
        if q:
            body["q"] = q
        res = await self._http.request("POST", f"{self._search_base}/{board_id}/search", json=body)
        return res.json()
