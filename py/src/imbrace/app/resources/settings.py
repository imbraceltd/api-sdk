...
class SettingsResource:
    """Settings domain — Sync. Message templates + user management."""
    ...
    def list_users(self, skip: int = 0, limit: int = 20, search: Optional[str] = None,
                   roles: Optional[str] = None, status: Optional[str] = None) -> Dict[str, Any]:
...
class AsyncSettingsResource:
    ...
    async def create_message_template(self, body: Dict[str, Any]) -> Dict[str, Any]:
        res = await self._http.request("POST", f"{self._base}/v1/backend/message_templates", json=body)
        return res.json()
