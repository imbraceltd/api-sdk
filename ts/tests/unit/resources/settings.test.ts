...
import { SettingsResource } from "../../../src/app/resources/settings.js"
...
function makeResource() {
  ...
  return new SettingsResource(http, BASE)
}
...
describe("SettingsResource", () => {
  ...
  it("listMessageTemplates() calls GET /v2/backend/message_templates", async () => {
    ...
    expect(url.pathname).toBe("/v2/backend/message_templates")
  ...
  it("listMessageTemplates() passes businessUnitId param", async () => {
    ...
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.searchParams.get("business_unit_id")).toBe("bu_1")
  })

  it("createMessageTemplate() calls POST /v1/backend/message_templates", async () => {
    mockFetch({ _id: "tpl_new" })
    await makeResource().createMessageTemplate({ name: "My Template", body: "Hello {{name}}" })
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.pathname).toBe("/v1/backend/message_templates")
    expect(vi.mocked(globalThis.fetch).mock.calls[0][1]?.method).toBe("POST")
  })

  it("deleteMessageTemplate() calls DELETE /v1/backend/message_templates/:id", async () => {
    mockFetch({})
    await makeResource().deleteMessageTemplate("tpl_1")
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.pathname).toBe("/v1/backend/message_templates/tpl_1")
    expect(vi.mocked(globalThis.fetch).mock.calls[0][1]?.method).toBe("DELETE")
  })

  it("listUsers() calls GET /v1/backend/users", async () => {
    mockFetch({ data: [{ id: "u_1", email: "user@example.com" }] })
    await makeResource().listUsers()
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.pathname).toBe("/v1/backend/users")
    expect(vi.mocked(globalThis.fetch).mock.calls[0][1]?.method).toBe("GET")
  })
  ...
  it("getUserRolesCount() calls GET /v1/backend/users/_roles_count", async () => {
    mockFetch({ admin: 2, agent: 10 })
    await makeResource().getUserRolesCount()
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.pathname).toBe("/v1/backend/users/_roles_count")
  })

  it("bulkInviteUsers() calls POST /v1/backend/users/_bulk_invite", async () => {
    mockFetch({ success: true })
    await makeResource().bulkInviteUsers({ emails: ["a@b.com"], role: "agent" })
    const url = new URL((vi.mocked(globalThis.fetch).mock.calls[0][0] as string))
    expect(url.pathname).toBe("/v1/backend/users/_bulk_invite")
    expect(vi.mocked(globalThis.fetch).mock.calls[0][1]?.method).toBe("POST")
  })
})
