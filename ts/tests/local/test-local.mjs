...
const BASE_URL =
  process.env.IMBRACE_GATEWAY_URL ?? "https://app-gateway.dev.imbrace.co";
...
// ── 3. Live API calls (skipped if no credentials)   

if (!API_KEY && !ACCESS_TOKEN) {
  console.log(
    "\n[3] Live API calls — SKIPPED (set IMBRACE_API_KEY or IMBRACE_ACCESS_TOKEN in tests/local/.env)",
  );
} else {
  ...
  const checks = [
    ...
    [
      "conversations.getViewsCount()",
      () => client.conversations.getViewsCount(),
    ],
