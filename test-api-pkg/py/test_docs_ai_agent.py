...
authenticated via API key. See test-api-pkg/ts/test-docs-ai-agent.ts for
section commentary.

Note on TS↔Py asymmetry called out in the doc:
  TS uses `client.boards.searchFolders/createFolder/updateFolder/deleteFolders`.
  Py mirrors knowledge-hub folders to a separate `client.folders` namespace
  (`folders.create/search/update/delete`) and uses `client.boards.upload_file`
  for actual file content. We exercise the Py-canonical surface here.
"""
...
print(f"\n=== DOCS: ai-agent.md - auth: API KEY (PyPI imbrace==1.0.4) ===")
...
if state["folderId"]:
    ...
    step("folders.delete ([id])",
         lambda: client.folders.delete([state["folderId"]]))
    state["folderId"] = None
else:
