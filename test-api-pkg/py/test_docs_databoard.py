...
— API-key auth. See test-api-pkg/ts/test-docs-databoard.ts for doc-gap
commentary.

Doc-gap call-outs:
  - Py docs `client.boards.create("Enterprise Leads")` — SDK 1.0.4 takes
    keyword args `client.boards.create(name="Enterprise Leads", description=...)`.
  - createItem shape `{ "fields": { "name", "status", "value" } }` returns 400.
    Canonical: `{ "fields": [{ "board_field_id", "value" }] }`.
  - updateItem shape `{ "fields": { "status" } }` returns 400. Canonical:
    `{ "data": [{ "key", "value" }] }`.
  - createField type lowercase (`"number"`) → 400. Canonical capitalised
    (`"Number"`, `"ShortText"`, ...).
"""
...
client = ImbraceClient(api_key=API_KEY, organization_id=ORG_ID, gateway=GATEWAY, timeout=15)
...
print(f"\n=== DOCS: databoard.md - auth: API KEY (PyPI imbrace==1.0.4) ===")
...
if state["boardId"]:
    ...
    skip("boards.bulk_delete_items", "needs real item ids — backend rejects empty list with 400")
else:
...
section("§3. Search")
if state["boardId"]:
    step("boards.search (q='enterprise')",
         lambda: client.boards.search(state["boardId"], q="enterprise"))
else:
