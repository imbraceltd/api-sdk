...
const client = new ImbraceClient({ apiKey, organizationId, baseUrl })
...
// Resolve an existing assistant to use for chat/prompt-suggestion tests
...
console.log(`\n━━━ aiAgent resource — auth: API KEY (npm @imbrace/sdk) ━━━`)
...
// Backend will fail to fetch the example.com URL — that's OK, we're verifying shape.
  ...
  await step("createClientChat", async () => {
    const r = await ai.createClientChat({
      ...
      message: {
        id: randomUUID(),
        role: "user",
        content: "hello from sdk verify (apikey)",
        createdAt: new Date().toISOString(),
        parts: [{ type: "text", text: "hello from sdk verify (apikey)" }],
      },
    })
