 ...
 * — Access Token auth. See test-api-pkg/ts/test-docs-databoard.ts for
 * doc-gap commentary.
 */
...
console.log(`\n━━━ DOCS: databoard.md — auth: ACCESS TOKEN (npm @imbrace/sdk@1.0.4) ━━━`)
...
if (state.boardId) {
  await step("boards.listItems (limit 100, empty)", () => client.boards.listItems(state.boardId!, { limit: 100 } as any))
  await step("boards.createField (canonical Type=ShortText) — find identifier", async () => {
    const u: any = await client.boards.createField(state.boardId!, { name: "Company", type: "ShortText" } as any)
...
note("doc-gap: databoard.md §4 createField uses `type: \"number\"` (lowercase) — SDK 1.0.4 backend expects `\"Number\"` (capitalised)")
...
if (state.boardId) {
  ...
  await step("boards.listSegments", () => (client.boards as any).listSegments(state.boardId!))
