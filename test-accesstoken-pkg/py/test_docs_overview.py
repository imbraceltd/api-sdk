...
print(f"\n=== DOCS: overview.md - auth: ACCESS TOKEN (PyPI imbrace==1.0.4) ===\n")

print("== §Hello world — context manager + platform.get_me ==")


def _hello():
    with ImbraceClient(access_token=ACCESS_TOKEN, organization_id=ORG_ID, gateway=GATEWAY, timeout=8) as c:
        return c.platform.get_me()


step("with ImbraceClient(access_token=...) as c: c.platform.get_me()", _hello)
