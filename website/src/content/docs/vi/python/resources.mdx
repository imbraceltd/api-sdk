---
title: Python SDK — Tính Năng
description: Hướng dẫn xây dựng CRM và AI với Imbrace Python SDK.
---

```python
from imbrace import ImbraceClient
client = ImbraceClient(access_token="acc_xxx")
```

---

## CRM

### Quản Lý Contacts

```python
# Danh sách contacts
contacts = client.contacts.list(limit=50).get("data", [])

# Lấy một contact
contact = client.contacts.get("contact_id")

# Cập nhật thông tin
client.contacts.update("contact_id", {
    "name":  "Alice B.",
    "email": "alice@example.com",
    "phone": "+84901234567",
})

# Lịch sử hoạt động, bình luận, file
activity = client.contacts.get_activities("conversation_id")
comments = client.contacts.get_comments("contact_id")
files    = client.contacts.get_files("contact_id")
```

---

### CRM Boards (Pipeline)

Boards là nơi lưu trữ dữ liệu CRM — leads, deals, tasks, hoặc bất kỳ dữ liệu có cấu trúc nào.

```python
# Danh sách boards
boards = client.boards.list().get("data", [])

# Lấy items của một board
leads = client.boards.list_items("board_id", limit=100).get("data", [])

# Tạo item mới
lead = client.boards.create_item("board_id", {
    "fields": {"name": "Acme Corp", "status": "new", "value": 50000}
})

# Cập nhật item
client.boards.update_item("board_id", lead["id"], {
    "fields": {"status": "qualified"}
})

# Tìm kiếm items
results = client.boards.search("board_id", {"query": "enterprise"})

# Xuất CSV
csv = client.boards.export_csv("board_id")
```

---

### Conversations

Quản lý cuộc hội thoại với khách hàng.

```python
# Tìm kiếm conversations
convs = client.conversations.search(
    business_unit_id="bu_xxx", q="support", limit=20
).get("data", [])

# Liệt kê tất cả conversations (có thể lọc)
all_convs = client.conversations.list(limit=50).get("data", [])

# Cập nhật trạng thái
client.conversations.update_status({
    "conversation_id": "conv_xxx",
    "status": "resolved",
})
```

---

### Nhắn Tin

Gửi và nhận tin nhắn qua các kênh (web, WhatsApp, Facebook, email, ...).

```python
# Danh sách kênh
channels = client.channel.list()

# Gửi tin nhắn
client.messages.send(type="text", text="Xin chào, tôi có thể giúp gì cho bạn?")

# Danh sách tin nhắn
msgs = client.messages.list(limit=20)
```

---

### Workflows & Automation

Kích hoạt channel automations để xử lý tin nhắn đến, phân luồng hội thoại và kích hoạt các hành động follow-up.

```python
# Danh sách tất cả channel automations
automations = client.workflows.list_channel_automation().get("data", [])

# Lọc theo loại kênh
whatsapp_flows = client.workflows.list_channel_automation(channel_type="whatsapp")
```

---

### Campaigns & Touchpoints

Quản lý chiến dịch marketing và các bước touchpoint của chiến dịch.

```python
# Campaign CRUD
campaigns = client.campaign.list().get("data", [])
campaign  = client.campaign.get("campaign_id")
new_camp  = client.campaign.create({"name": "Q2 Outreach", "type": "email"})
client.campaign.delete("campaign_id")

# Touchpoints trong chiến dịch
touchpoints = client.campaign.list_touchpoints().get("data", [])
tp = client.campaign.get_touchpoint("touchpoint_id")

client.campaign.create_touchpoint({
    "campaign_id": "campaign_id",
    "type":        "email",
    "delay_days":  3,
})
client.campaign.update_touchpoint("touchpoint_id", {"delay_days": 5})
client.campaign.delete_touchpoint("touchpoint_id")

# Kiểm tra cấu hình touchpoint trước khi lưu
result = client.campaign.validate_touchpoint({"type": "email", "template_id": "tpl_xxx"})
```

---

### Gợi Ý Tin Nhắn

Gợi ý câu trả lời tự động dựa trên ngữ cảnh hội thoại.

```python
suggestions = client.message_suggestion.get_suggestions({
    "conversation_id": "conv_xxx",
    "limit": 3,
})
```

---

### Dự Đoán (Predict)

Gửi dữ liệu đầu vào đến dịch vụ dự đoán để chấm điểm hoặc phân loại bằng ML.

```python
result = client.predict.predict({
    "model": "lead_score_v1",
    "input": {"company_size": 200, "industry": "saas", "mrr": 5000},
})
print(result["score"])   # 0.87
```

---

## AI

### AI Service (aiv2)

`client.ai` và `client.chat_ai` đều gọi đến dịch vụ **aiv2**. Dùng `client.ai` cho các lệnh gọi OpenAI-compatible; dùng `client.chat_ai` cho phiên chat và xử lý tài liệu.

```python
# ── client.ai — OpenAI-compatible completions ──────────────────────────────

response = client.ai.complete(
    model="gpt-4o",
    messages=[
        {"role": "system", "content": "Bạn là trợ lý CRM."},
        {"role": "user",   "content": "Tóm tắt ghi chú khách hàng này: ..."},
    ],
)
print(response["choices"][0]["message"]["content"])

for chunk in client.ai.stream(
    model="gpt-4o",
    messages=[{"role": "user", "content": "Soạn email follow-up cho lead này."}],
):
    print(chunk["choices"][0]["delta"].get("content", ""), end="", flush=True)

result = client.ai.embed(
    model="text-embedding-ada-002",
    input=["khách hàng khiếu nại về billing", "vấn đề billing leo thang"],
)

# ── client.chat_ai — managed chat sessions ────────────────────────────────

models    = client.chat_ai.list_models()
response2 = client.chat_ai.chat({
    "model": "gpt-4o",
    "messages": [{"role": "user", "content": "Chính sách hoàn tiền là gì?"}],
})
chat    = client.chat_ai.create_chat({"title": "Support Chat"})
history = client.chat_ai.list_chats()
client.chat_ai.delete_chat(chat["id"])

# ── Hiểu Tài Liệu ─────────────────────────────────────────────────────────

result = client.chat_ai.process_document(
    model_name="gpt-4o",
    url="https://example.com/invoice.pdf",
    organization_id="org_xxx",  # bắt buộc cho endpoint này
)
print(result["data"])
# {"invoice_number": "INV-001", "total": 1200, "vendor": "Acme Corp", ...}
```

---

### AI Agent

Chat streaming với knowledge base và quản lý phiên chat end-user.

```python
import uuid

# Stream chat với AI assistant (SSE)
# id là session/conversation ID — truyền cùng id qua các lượt để giữ lịch sử hội thoại.
#   Chỉ bỏ qua khi bắt đầu cuộc trò chuyện mới; SDK sẽ tự tạo nếu không truyền.
# user_id là tuỳ chọn — được lấy tự động qua chat-client/auth/user khi không cung cấp.
session_id = str(uuid.uuid4())  # lưu lại id này cho toàn bộ cuộc trò chuyện

resp = client.ai_agent.stream_chat({
    "id": session_id,
    "assistant_id": "assistant_id",
    "organization_id": "org_xxx",
    "messages": [{"role": "user", "content": "Deals nào đã chốt quý này?"}],
})
for line in resp.iter_lines():
    print(line)

# Gợi ý câu hỏi
suggestions = client.ai_agent.get_agent_prompt_suggestion("assistant_id")

# Knowledge base
files      = client.ai_agent.list_embedding_files()
classified = client.ai_agent.classify_file(
    url="https://example.com/product-catalog.pdf",
    mime_type="application/pdf",
)

# Quản lý phiên chat end-user
from datetime import datetime, timezone
chat = client.ai_agent.create_client_chat({
    "id": str(uuid.uuid4()),
    "assistantId": "assistant_id",
    "organizationId": "org_xxx",
    "userId": "user_id",               # lấy từ chat-client/auth/user
    "selectedVisibilityType": "private",
    "message": {
        "id":        str(uuid.uuid4()),
        "role":      "user",
        "content":   "Xin chào",
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "parts":     [{"type": "text", "text": "Xin chào"}],
    },
})

messages = client.ai_agent.list_client_messages(chat["id"])
client.ai_agent.delete_client_chat(chat["id"])
```
