...
description: Tham chiếu cho chatAi / chat_ai (quản lý assistant), aiAgent / ai_agent (streaming chat, embeddings, parquet, tracing) và OpenAI-compatible service client.ai.
---
...
- **`chatAi` / `chat_ai`** — tạo và quản lý AI assistants, chạy completions, quản lý knowledge bases.
...
Để xem ví dụ end-to-end kết nối hai phần này, xem [Hướng Dẫn Luồng Đầy Đủ §1](/vi/sdk/full-flow-guide/#1-tạo-ai-assistant-và-bắt-đầu-chat).
...
Folders và knowledge bases được quản lý qua `client.boards` (TypeScript) / `client.folders` và `client.boards` (Python). Truyền folder IDs vào `folder_ids` khi tạo assistant — xem [Hướng Dẫn Luồng Đầy Đủ §3](/vi/sdk/full-flow-guide/#3-quản-lý-knowledge-hub-và-gắn-vào-assistant).
