...
3. **Stream phản hồi chat từ AI agent**
...
## 2. Tạo Workflow và Liên Kết với AI Agent
...
5. **Liên kết flow với AI agent**

   Mở AI agent trong dashboard Imbrace, vào **Tools → Workflows** và gắn flow vào. AI agent sẽ có thể kích hoạt flow trong quá trình hội thoại khi phù hợp.
...
Knowledge Hub chứa files và folders trong service **data-board** (`client.boards`). `_id` của folder là giá trị bạn truyền vào AI agent làm nguồn tri thức.
...
3. **Gắn folder vào AI agent**

   Truyền `_id` của folder vào `folder_ids` — AI agent sẽ truy xuất từ mọi file trong folder đó. Dùng `board_ids` để gắn thêm một CRM data-board (các item của board trở thành nguồn tri thức). Trường `knowledge_hubs` cũ đã ngừng hỗ trợ.
