...
Imbrace CLI là công cụ dòng lệnh sử dụng proxy API cục bộ để giao tiếp với nền tảng.

```bash
# Cài đặt global
npm install -g @imbrace/cli

# Khởi động server API cục bộ (giữ chạy)
cd api
bun install
...
Xem [CLI Installation](/cli/installation/) để biết thêm cách cài đặt và [CLI Commands](/cli/commands/) để tham khảo lệnh đầy đủ.
