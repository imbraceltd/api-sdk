...
3. **Kích hoạt flow thủ công với payload**
...
3. **Gắn folder vào assistant**
   ...
   Truyền `_id` của folder vào `knowledge_hubs` — assistant sẽ dùng tất cả files trong folder đó.

   ```typescript
   await client.chatAi.updateAssistant(assistantId, {
     name: "Support Bot",
     workflow_name: "support_bot_v1",
     knowledge_hubs: [folder._id],
   })
   ```
