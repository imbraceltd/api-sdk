---
title: 完整流程指南
description: 端到端指南 — 從初始化 client 到 AI agents、工作流程、knowledge hubs 和資料看板。TypeScript 和 Python。
---
...
5. **將 flow 與 AI Agent 關聯**

   在 Imbrace dashboard 中開啟 AI Agent，進入 **Tools → Workflows** 並綁定 flow。AI Agent 在對話中將能夠在適當時機觸發 flow。

   或更新 AI Agent 以按名稱引用工作流程：
...
## 3. 管理 Knowledge Hub 並綁定到 AI Agent

Knowledge Hub 在 **data-board** 服務（`client.boards`）中儲存檔案和資料夾。資料夾的 `_id` 是您傳給 AI Agent 作為知識來源的值。
...
3. **將資料夾綁定到 AI Agent**
   ...
   將資料夾的 `_id` 傳入 `folder_ids` — AI Agent 將從該資料夾的所有檔案中檢索。使用 `board_ids` 可額外附加 CRM data-board。舊的 `knowledge_hubs` 欄位已停用。
