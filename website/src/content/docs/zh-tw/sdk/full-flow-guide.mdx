...
本指南從頭到尾逐步說明 Imbrace SDK 中的四大主要工作流程。每個章節都是獨立的 — 可以依序閱讀或直接跳到您需要的部分。切換語言標籤一次，頁面其餘部分將記住您的選擇。
...
1. **初始化客戶端**
   ...
   頁首下拉選單可在**存取令牌**（Imbrace 作為後端的面向使用者應用程式）和**API 金鑰**（Imbrace 作為您技術棧內部功能的服務對服務）之間切換這些程式碼片段。請參閱[認證 → 應該使用哪種憑證](/sdk/authentication/#which-credential-should-i-use)以了解完整的決策樹。
