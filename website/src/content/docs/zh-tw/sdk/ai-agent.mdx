...
description: chatAi / chat_ai（助手管理）、aiAgent / ai_agent（串流聊天、embeddings、parquet、追蹤）以及 OpenAI 相容 client.ai 服務的參考文件。
---
...
- **`chatAi` / `chat_ai`** — 建立和管理 AI 助手、執行 completions、管理 knowledge bases。
...
資料夾和 knowledge bases 透過 `client.boards`（TypeScript）/ `client.folders` 和 `client.boards`（Python）管理。建立助手時在 `folder_ids` 中傳入資料夾 ID — 參閱[完整流程指南 §3](/zh-tw/sdk/full-flow-guide/#3-管理-knowledge-hub-並綁定到助手)。
