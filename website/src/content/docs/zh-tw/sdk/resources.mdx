---
title: 資源參考
description: Imbrace SDKs 的按命名空間參考 — contacts、conversations、messaging、campaigns 等。
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

本頁涵蓋 SDK 公開的 CRM 和訊息命名空間。首先初始化 client（參閱[安裝](/zh-tw/sdk/installation/)或[快速入門](/zh-tw/sdk/quick-start/)）。以下所有程式碼片段均假設 `client` 是已初始化的實例。

AI 和工作流程相關資源，請參閱：
- [AI Agent](/zh-tw/sdk/ai-agent/) — 助手（`chatAi`）、AI agent 串流、embeddings、parquet
- [工作流程](/zh-tw/sdk/workflows/) — Activepieces flows 和頻道自動化
- [資料看板](/zh-tw/sdk/databoard/) — CRM 管道看板和項目
...
const newCamp   = await client.campaign.create({ name: "Q2 推廣", type: "email" });
