---
title: 完整流程指南
description: 端到端指南 — 從初始化 client 到 AI agents、工作流、knowledge hubs 和資料看板。
---

import { Steps } from '@astrojs/starlight/components';

本指南從頭到尾演示 Imbrace SDK 的四個主要流程。每個部分相互獨立 — 可以按順序閱讀，也可以直接跳到需要的部分。
...
2. **建立助手**
   ...
   `provider_id` 與 `model_id` 為必填。傳入 `provider_id: "system"` 可
   使用組織預設的 LLM 供應商，或傳入自訂供應商的 UUID。當
   `provider_id: "system"` 時，`model_id` 可填模型名稱（例如
   `"gpt-4o"`）或字串 `"Default"` 以使用系統預設模型。
...
3. **新增 Webhook 觸發器並發布 flow**
   ...
   剛建立的 flow 處於 **DRAFT** 狀態且沒有觸發器——webhook URL
   尚未存在，因此呼叫 `triggerFlow` 會回傳 404。新增 Webhook piece
   作為觸發器，再發布：
...
5. **將 flow 與助手關聯**
   ...
   await client.chatAi.updateAssistant(assistantId, {
     name: "Support Bot",
     workflow_name: "support_bot_v1",
     // workflow_function_call 將助手與 Activepieces flows 關聯
...
3. **將資料夾綁定到助手**
   ...
   將資料夾的 `_id` 傳入 `folder_ids` — 助手將從該資料夾的所有檔案中檢索。
   使用 `board_ids` 可額外附加 CRM data-board（其 items 將成為知識來源）。
   舊的 `knowledge_hubs` 欄位已停用。
...
1. **建立看板**
   ...
   看板是一個 CRM pipeline — leads、deals、tasks 或任何結構化資料。
...
2. **新增自訂欄位**
   ...
   欄位類型：`ShortText`、`LongText`、`Number`、`Dropdown`、`Date`、`Checkbox` 等。
   `createField` 返回已更新的看板 — 新欄位位於 `board.fields` 中。
