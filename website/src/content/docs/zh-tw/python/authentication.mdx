...
SDK 支援兩種憑證類型。向建構函式傳入 `api_key` 或 `access_token` — transport 自動處理請求標頭。
...
Org context **內嵌在憑證本身** — 每個 API key 與每個 access token 都綁定到唯一一個 org。Gateway 從你送出的憑證即可解析 org，**完全不需要**傳 `organization_id` 給 SDK。
