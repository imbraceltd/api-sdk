...
> Already installed? Skip to [Authentication](/typescript/authentication/) for credential strategy or [Quick Start](/typescript/quick-start/) for a working call in 60 seconds.
...
Pass credentials directly to the constructor. Use a `.env` file with `dotenv` or your framework's env loader to manage secrets — the SDK does not read them automatically.
...
> Not sure which one you need? See [Authentication](/typescript/authentication/) — the choice depends on whether Imbrace is your backend (Access Token) or a feature inside your stack (API Key).
...
For your first working call see [TypeScript Quick Start](/typescript/quick-start/) or [Python Quick Start](/python/quick-start/). For the full end-to-end walkthrough (assistants, workflows, knowledge hub, boards), see [Full Flow Guide](/typescript/full-flow-guide/).
...
For runtime auth and API errors, see [Troubleshooting](/guides/troubleshooting/) and [Error Handling](/typescript/error-handling/).
