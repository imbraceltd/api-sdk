...
1. **Initialize the client**
   ...
   You can pass either an **access token** (for user-facing apps where Imbrace is your backend) or an **api key** (for service-to-service where Imbrace is a feature inside your stack). See [Authentication → which credential to use](/typescript/authentication/#which-credential-should-i-use) for the full decision tree.
...
2. **Create an assistant**
   ...
   to the system default model. For full assistant CRUD, see
   [Resources → chatAi](/typescript/resources/#chatai).
...
3. **Attach the folder to the assistant**
   ...
   become a knowledge source — see [Resources → boards](/typescript/resources/#boards)).
   The legacy `knowledge_hubs` field is deprecated.
