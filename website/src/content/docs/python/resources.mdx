...
chat  = client.chat_ai.get_chat("chat_id")
...
res    = client.ai_agent.list_chats(organization_id="org_xxx", limit=10)
chats  = res.get("chats", [])
chat   = client.ai_agent.get_chat(chats[0]["id"])

# Chat v2 — streaming (returns raw httpx.Response for SSE)
...
classified = client.ai_agent.classify_file(url="https://…/doc.pdf", mime_type="application/pdf")

# Parquet
result = client.ai_agent.generate_parquet(
    data=[{"id": 1, "value": 42}],
    file_name="export_2026",
)
client.ai_agent.delete_parquet_file(result["fileName"])

# Chat client (end-user chats)
import uuid
new_chat = client.ai_agent.create_client_chat({
    "id": str(uuid.uuid4()),
...
messages = client.ai_agent.list_client_messages(new_chat["id"])
client.ai_agent.delete_client_chat(new_chat["id"])
```
