...
在项目根目录创建 `.env` 文件。SDK **不会**自动读取环境变量——你需要在第 3 步将其传入构造函数。
...
如需了解何时使用 API Key 或 Access Token，请参阅 [Authentication](/sdk/authentication/)；如需了解如何获取 API Key，请参阅 [Setup 指南](/getting-started/setup/#get-an-api-key)。
