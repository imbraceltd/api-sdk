---
title: AI Agent
description: chatAi / chat_ai（AI Agent 管理）、aiAgent / ai_agent（流式聊天、embeddings、parquet、追踪）以及 OpenAI 兼容 client.ai 服务的参考文档。
---
...
- **`chatAi` / `chat_ai`** — 创建和管理 AI Agent、运行 completions、管理 knowledge bases。
...
端到端 `streamChat` 示例，参阅[完整流程指南 §1](/zh-cn/sdk/full-flow-guide/#1-创建-ai-agent-并开始聊天)。
...
管理 AI Agent（CRUD）、运行 OpenAI 兼容的 completions，以及处理 document/file。该命名空间也涵盖 Knowledge Hub 文件夹和 knowledge bases。
...
文件夹和 knowledge bases 通过 `client.boards`（TypeScript）/ `client.folders` 和 `client.boards`（Python）管理。创建 AI Agent 时在 `folder_ids` 中传入文件夹 ID — 参阅[完整流程指南 §3](/zh-cn/sdk/full-flow-guide/#3-管理-knowledge-hub-并绑定到-ai-agent)。
...
管理用于检索增强生成（RAG）的文件。先通过 [`client.boards.uploadFile`](/zh-cn/sdk/full-flow-guide/#3-管理-knowledge-hub-并绑定到-ai-agent) 上传文件，再触发 embedding 处理。
