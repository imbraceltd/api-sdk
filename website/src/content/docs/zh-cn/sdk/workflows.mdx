---
title: 工作流
description: Workflows — 可视化工作流构建器和渠道自动化 — 通过 SDK 创建、触发和管理 flows。
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

SDK 公开两个工作流系统：**Workflows**（`client.workflows`）用于可视化工作流自动化，**渠道自动化**（`client.workflows`）用于消息渠道流程（如 WhatsApp）。

首先初始化 client（参阅[安装](/zh-cn/sdk/installation/)或[快速入门](/zh-cn/sdk/quick-start/)）。端到端演练请参阅[完整流程指南 §2](/zh-cn/sdk/full-flow-guide/#2-创建工作流并与-ai-agent-关联)。

---

## Workflows — `client.workflows`

可视化工作流构建器。TypeScript 和 Python 均可使用。
...
const folder             = await client.workflows.createFolder({ displayName: "CRM 自动化", projectId: "project_id" });
