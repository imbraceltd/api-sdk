...
The Imbrace SDK is the official client for the Imbrace Gateway, available in **TypeScript** and **Python**. Both SDKs wrap the same Gateway API with the same resource namespaces, the same auth model, and the same retry/error semantics â€” pick whichever language fits your stack.
...
| **Two credential types** | `apiKey` or `accessToken` â€” see [Authentication](/sdk/authentication/) |
...
Every namespace is on both SDKs. Methods follow the language conventions â€” `client.aiAgent.streamChat()` in TS, `client.ai_agent.stream_chat()` in Python.
...
| `client.activepieces` | Workflow automation â€” flows, triggers, runs |
| `client.boards` | CRM boards â€” CRUD, items, fields, search, segments, CSV; KH folders & files |
...
Full decision tree: [Authentication â†’](/sdk/authentication/).

### Next steps

- [Installation â†’](/sdk/installation/) â€” set up the package and credentials
- [Quick Start â†’](/sdk/quick-start/) â€” make your first call in 60 seconds
- [Full Flow Guide â†’](/sdk/full-flow-guide/) â€” end-to-end walkthrough of the four major workflows (assistants, workflows, knowledge hub, boards)